...
SELECT f.facid, CONCAT( m.firstname, ' ', m.surname ) AS m_name,
CASE WHEN m.memid =0
...
SELECT m_name, f_name, cost
FROM (

SELECT CONCAT( m.firstname, ' ', m.surname ) AS m_name, f.name AS f_name,
CASE
WHEN m.memid =0
THEN b.slots * f.guestcost
ELSE b.slots * f.membercost
